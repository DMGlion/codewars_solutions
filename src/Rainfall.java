...
public class Rainfall {
    ...
    public static double mean(String town, String strng) {
        // your code
        List<Double> values = filterValuesDoubleAndCity(town, strng);
    ...
    private static double calculateMean(List<Double> values) {
        ...
        return resultAVG/values.size();
    }

    private static List<Double> filterValuesDoubleAndCity(String town, String strng) {
        List<Double> valuesNoComa = new ArrayList<>();
        List<String> allValues = new ArrayList<>(Arrays.asList(strng.split("[\n|\r]")));
        Stream<String> oneCity = allValues.stream().filter(citi-> citi.startsWith(town+":"));
        List<String> value = oneCity.collect(Collectors.toList());
        if (value.size()==0){
            return valuesNoComa;
        }
        List<String> valueComa = new ArrayList<>(Arrays.asList(value.get(0).split(",")));
        for (String s: valueComa){
            valuesNoComa.add(Double.valueOf(s.replaceAll("[^\\d.]", "")));
        }
        return valuesNoComa;
    }
    ...
    public static double variance(String town, String strng) {
        ...
        double avgNean = calculateMean(values);

        //calculate variance
        double avgVariance = calculateVariance(avgNean, values);
        return avgVariance;
    }

    private static double calculateVariance(double avgNean, List<Double> values) {
        double varianceSum = 0;
        for (double s: values){
            varianceSum+=(Math.pow(s-avgNean, 2));
        }
    ...
    public static void main(String[] args) {
        System.out.println(mean("Berlin", data));
